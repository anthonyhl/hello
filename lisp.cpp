...
#include <stdio.h>  //fflush(), fopen(), fclose(), fprintf(), sprintf(), fgetc(), 
#include <string.h> //strlen(), strcpy(), strcmp()
#include <stdlib.h> //calloc(),
#include <math.h>   //labs(), floor(), and pow()
...
struct Atomtable 
{
    char name[16]; int32 L; int32 bl; int32 plist;
}; 
...
{
  public:
    G() : nf(-1), P(0), fp(-1), pb(0), ct(0), tracesw(0) 
    {
      ...
      ----------------------------------------------------------------------------*/
{
    ...
    for (;;) {
        ...
        r=i.sread();
        r=i.seval(r);
        i.swrite(r);  /* swrite uses/frees no list-nodes. */
    }
      ...
      -----------------------------------------------------------------------------*/
{
    ...
    {
sprompt: 
        if (filep EQ stdin) {sprintf(sout,"%c",prompt); ourprint(sout);}
      ...
      -----------------------------------------------------------------------------*/
{
    ...
    *start = '\0';
    ...
    //{if (c EQ '\t' OR c EQ '\r') c= BLANK; s[i]= c;}
    //s[i]= '\0';
    //if (c EQ EOF AND i EQ 0) return(-1); else return(i);
}

/*===========================================================================*/
int32 G::numatom(double r)
    /*----------------------------------------------------------------------------
...
/*===========================================================================*/
void G::traceprint(int32 v, int16 osw)
...
/*========================================================================*/
int32 G::newloc(int32 x, int32 y)
    /*--------------------------------------------------------------------------
...
start:
    ...
    if (listp(t))
    {
        ...
        if (NOT listp(type(t)))
        {
            marknum(type(t),t); p=B(p);
            goto start;
        }
        s=B(p); 
        if (NOT listp(type(s)))
        {
            marknum(type(s),s); p=t; 
            goto start;
        }
        gcmark(t); 
        p=B(p); 
